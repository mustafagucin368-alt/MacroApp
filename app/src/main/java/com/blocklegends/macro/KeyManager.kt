...
object KeyManager {
    ...
    fun getActiveKey(context: Context): String? =
        try { getSecurePrefs(context).getString(KEY_ACTIVE, null) } catch (e: Exception) { null }

    fun getExpiry(context: Context): Long =
        try { getSecurePrefs(context).getLong(KEY_EXPIRY, 0L) } catch (e: Exception) { 0L }
    ...
    fun validateAdmin(context: Context, password: String): Boolean =
        try {
            ...
            password == stored
        } catch (e: Exception) { password == DEFAULT_ADMIN }
